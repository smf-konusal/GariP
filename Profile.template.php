...
function template_alerts_popup()
{
	...
	// Unlike almost every other template, this is designed to be included into the HTML directly via $().load()
	echo '<div class="alert_bar animated--grow-in" >
				<div class="alerts_opts block">
								...
								else
								{
									...
									{
										echo '
										<', !$details['show_links'] ? 'a href="' . $scripturl . '?action=profile;area=showalerts;alert=' . $id_alert . '" onclick="this.classList.add(\'alert_read\')"' : 'div', ' class="d-flex align-items-center">
											<div class="mr-3 unread_notify_image">
												', empty($details['sender']['avatar']['image']) ? '' : $details['sender']['avatar']['image'] . '
												', $details['icon'], '
											</div>
											<div class="details">
												<div class="small text-gray-600">', $details['time'], '</div>
												<span class="alert_text">', $details['text'], '</span> - 
											</div>
				...
				<a href="', $scripturl, '?action=profile;area=showalerts" class="small text-gray-700">', $txt['all_alerts'], '</a>
			</div>
		</div>
		<script>
		...
		<div class="container">
			<div class="row">
				<div class="col-md-6 mt-3">
					<div class="card shadow mb-4">
                         <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">',$txt['summary'],'</h6>
                        </div>
                        <div class="card-body">
							<div class="card garipstyle">
								',$context['member']['avatar']['image'],'
								<p>', $context['member']['name'],'</p>
								...
								if (!isset($context['disabled_fields']['posts']))
									echo '
											<dt class="col-sm-6">', $txt['profile_posts'], ': </dt>
											<dd class="col-sm-6">', $context['member']['posts'], ' (', $context['member']['posts_per_day'], ' ', $txt['posts_per_day'], ')</dd>';

								if ($context['member']['show_email'])
									echo '
											<dt class="col-sm-6">', $txt['email'], ': </dt>
											<dd class="col-sm-6"><a href="mailto:', $context['member']['email'], '">', $context['member']['email'], '</a></dd>';

								if (!empty($modSettings['titlesEnable']) && !empty($context['member']['title']))
									echo '
											<dt class="col-sm-6">', $txt['custom_title'], ': </dt>
											<dd class="col-sm-6">', $context['member']['title'], '</dd>';

								if (!empty($context['member']['blurb']))
									echo '
											<dt class="col-sm-6">', $txt['personal_text'], ': </dt>
											<dd class="col-sm-6">', $context['member']['blurb'], '</dd>';

								echo '
											<dt class="col-sm-6">', $txt['age'], ':</dt>
											<dd class="col-sm-6">', $context['member']['age'] . ($context['member']['today_is_birthday'] ? ' &nbsp; <img src="' . $settings['images_url'] . '/cake.png" alt="">' : ''), '</dd>';
								...
								if (!empty($context['print_custom_fields']['standard']))
								{
									...
									if (count($fields) > 0)
									{
										...
										foreach ($fields as $field)
											echo '
											<dt class="col-sm-6">', $field['name'], ':</dt>
											<dd class="col-sm-6">', $field['output_html'], '</dd>';
								...
								echo '
										<dl class="row noborder">';

								// Can they view/issue a warning?
								if ($context['can_view_warning'] && $context['member']['warning'])
								{
									echo '
											<dt class="col-sm-6">', $txt['profile_warning_level'], ': </dt>
											<dd class="col-sm-6">
								...
								if (!empty($context['activate_message']) || !empty($context['member']['bans']))
								{
									...
									if (!empty($context['member']['bans']))
									{
										echo '
											...
											<dt class="col-sm-6" class="clear hidden" id="ban_info">
								...
								echo '
											<dt class="col-sm-6">', $txt['date_registered'], ': </dt>
											<dd class="col-sm-6">', $context['member']['registered'], '</dd>';
								...
								if ($context['can_see_ip'])
								{
									if (!empty($context['member']['ip']))
										echo '
											<dt class="col-sm-6">', $txt['ip'], ': </dt>
											<dd class="col-sm-6"><a href="', $scripturl, '?action=profile;area=tracking;sa=ip;searchip=', $context['member']['ip'], ';u=', $context['member']['id'], '">', $context['member']['ip'], '</a></dd>';

									if (empty($modSettings['disableHostnameLookup']) && !empty($context['member']['ip']))
										echo '
											<dt class="col-sm-6">', $txt['hostname'], ': </dt>
											<dd class="col-sm-6">', $context['member']['hostname'], '</dd>';
								}

								echo '
											<dt class="col-sm-6">', $txt['local_time'], ':</dt>
											<dd class="col-sm-6">', $context['member']['local_time'], '</dd>';

								if (!empty($modSettings['userLanguage']) && !empty($context['member']['language']))
									echo '
											<dt class="col-sm-6">', $txt['language'], ':</dt>
											<dd class="col-sm-6">', $context['member']['language'], '</dd>';

								if ($context['member']['show_last_login'])
									echo '
											<dt class="col-sm-6">', $txt['lastLoggedIn'], ': </dt>
											<dd class="col-sm-6">', $context['member']['last_login'], (!empty($context['member']['is_hidden']) ? ' (' . $txt['hidden'] . ')' : ''), '</dd>';
				...
				<div class="col-md-6 mt-3">
					<div class="card shadow mb-4">
						<div class="card-header py-3">
							<h6 class="m-0 font-weight-bold text-primary">',$txt['statPanel_showStats'],'</h6>
						</div>
						<div class="card-body">';
							require_once($sourcedir . '/Profile-View.php');
							statPanel($context['id_member']);
								echo '
									<div id="generalstats">
										<dl class="row">';
								...
								{
									echo '
											<dt class="col-sm-6">', $txt['statPanel_' . $key], '</dt>';

									if (!empty($stat['url']))
										echo '
											<dd class="col-sm-6"><a href="', $stat['url'], '">', $stat['text'], '</a></dd>';
									else
										echo '
											<dd class="col-sm-6">', $stat['text'], '</dd>';
								}
								...
								echo '
										...
										<div class="btn btn-info btn-icon-split btn-sm">
												<span class="icon main_icons replies"></span> <span class="text">', $txt['statPanel_topBoardsActivity'], '</span>
										</div>';
							...
							else
							{
								...
								{
									echo '
											<dt class="col-sm-6">', $activity['link'], '</dt>
											<dd class="col-sm-6">
							...
							echo'			
									<div class="btn btn-warning btn-icon-split btn-sm">
												<span class="icon main_icons replies"></span> <span class="text">', $txt['statPanel_topBoards'], '</span>
									</div>';
								...
								else
								{
									...
									{
										echo '
												<dt class="col-sm-6">', $board['link'], '</dt>
												<dd class="col-sm-6">
